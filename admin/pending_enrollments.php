...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("UPDATE course_enrollment SET status=? WHERE id=?");
...
.course-card .actions { display:flex; justify-content:center; gap:10px; margin-top:10px; }
.course-card .actions a { padding:10px 15px; color:white; text-decoration:none; border-radius:6px; transition:0.3s; }
.course-card .approve { background:#28a745; }
.course-card .approve:hover { background:#218838; }
.course-card .reject { background:#dc3545; }
.course-card .reject:hover { background:#c82333; }
...
<div class="sidebar">
  ...
  <a href="pending_enrollments.php" class="active">⏳ Pending</a>
  <a href="approved_enrollments.php">✅ Approved</a>
...
<div class="main-content">
  <div class="navbar">
    <div class="logo"><img src="../images/login.jpg" alt="Logo"> Pending Enrollments</div>
      ...
      <?php if($enrollments->num_rows > 0): ?>
        <?php while($row = $enrollments->fetch_assoc()): ?>
          <div class="course-card">
            ...
            <p><strong>Applied On:</strong> <?= date("d-m-Y h:i A", strtotime($row['created_at'])); ?></p>
            <div class="actions">
        ...
        <?php endwhile; ?>
      <?php else: ?>
        <p style="text-align:center; font-size:18px; color:#444;">No pending enrollments.</p>
      <?php endif; ?>
    </div>
  </div>
